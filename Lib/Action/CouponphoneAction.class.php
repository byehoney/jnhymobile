...
//		$resp = $this->api_ActivityFind(json_encode($data));
  	 	$resp = M("Cxactivity")->order("id DESC")->limit(10)->select();
		$this->debug($resp);
	}
	/**
	 * 创建消费送
...
//			$list[$k]['end_time'] = date("Y-m-d",$v['end_time']);
			...
			if($begin_date<=$time){
				...
				file_put_contents("./log/api_indicatorQuery.txt","==>".json_encode($_SESSION)."==>".json_encode($result)."\t\r\n",FILE_APPEND);
				if($result["code"]!=0){
...
//		$this->debug($before_act_avag_amt);
		...
		if(!$upload->upload()) {// 上传错误提示错误信息
			...
			if(file_exists($filePath)){ //上传到图片空间
				...
				if(!$res['code']){
					...
					file_put_contents("./log/".date("Ymd")."_img.txt",json_encode($data)."\t\r\n",FILE_APPEND);
					$this->ajaxReturn($data,'上传成功！',1);
				}else{
					file_put_contents("./log/".date("Ymd")."error_img.txt",json_encode($res)."\t\r\n",FILE_APPEND);
	...
	//图片上传
	public function ajaxUploadImagesData(){
		$this->_imgPath = $this->_imgPath.substr($_SESSION['seller_id'],0,7)."/".$_SESSION['seller_id']."/";
		$basedata = $_POST['logo'];
		$logoArr = explode(";base64,",$basedata);//获取到的base64 整体图片信息
		$logoData = base64_decode($logoArr[1]);//解析图片信息
		$logoHeader = $logoArr[0];//获取到的base64头部信息
		$logoHeaderArr= explode("image/",$logoHeader);//将头部信息拆分成数组
		$imgSavename = time();//给图片命名
		$logo_path = $this->_imgPath.$imgSavename.".".$logoHeaderArr[1];//拼接图片相对路径
		if(!is_dir($this->_imgPath)){
			mkdir($this->_imgPath);
		}
		//	$this->debug($logo_path,0,0);
		file_put_contents($logo_path,$logoData);
		if(!is_file($logo_path)) {// 上传错误提示错误信息
			$this->ajaxReturn(null,"上传错误",0);
		}else{// 上传成功 获取上传文件信息
			$filePath = $logo_path;
			if(file_exists($filePath)){ //上传到图片空间
				$filename = '@'.APP_ROOT.$filePath;
				$filename = str_replace('\\', '/', str_replace('./', '/', $filename));
				$res=$this->api_uploadImage($imgSavename,$filename,$logoHeaderArr[1]);
				if(!$res['code']){
					//unlink($filePath);
					$data['image_path']=$filePath;
					$data['image_id']=$res['image_id'];
					...
					file_put_contents("./log/".date("Ymd")."_error_img.txt",json_encode($res)."\t\r\n",FILE_APPEND);
					if($res['code_msg']=='应用授权令牌已过期'){
						$resData = array('sq'=>true,'url'=>$this->_authUrl);
					}else{
						$resData = array();
